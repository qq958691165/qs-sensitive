...
class Senstive{
     ...
     * $words 自定义敏感词文件库
     */
    public function __construct($disturbList = [], $words = '')
    {
        if($disturbList){
            $this->interference($disturbList);
        }
        else{
            $this->interference(['&', '*']);
        }
        ...
        if($words){
            ...
            $this->addWords(__DIR__ . '/../asset/words.txt');
        }
    }

    protected function interference($disturbList = array())
    {
     ...
     */
    protected function addWords($filename)
    {
